...
func spawn_particles(type: String, position: Vector2) -> void:
	...
	match type:
		...
		"jitter_spark":
			_spawn_spark_particles(position, Color(0.9, 0.7, 0.2), 6)
		"electric_sparks":
			_spawn_spark_particles(position, Color(0.8, 0.9, 1.0), 12)
		"dust_motes":
			_spawn_dust_particles(position, Color(0.7, 0.7, 0.6), 20)
		"glitch_fragments":
			_spawn_glitch_particles(position, Color(0.9, 0.2, 0.9), 15)
...
func _spawn_dust_particles(pos: Vector2, color: Color, amount: int) -> void:
	...
	material.emission_box_extents = Vector3(50, 50, 0)
	material.spread = 180.0
